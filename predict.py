...
PATH_TO_MODELS = cwd + '/models/model.pkl'
...
def main():
    '''функция для запуска предобработки сырых данных и прогноза на 14 дней вперёд согласно  запросу sales_submitions'''
    sales_df_train = pd.read_csv(PATH_SALES_DF_TRAIN)
    pr_df = pd.read_csv(PATH_PR_DF)
    st_df = pd.read_csv(PATH_ST_DF)
    sales_submission_df = pd.read_csv(PATH_TO_SUMISSION_DF)
    list_holidays = (pd.read_csv(PATH_TO_HOLIDAYS)['holidays'].values)
    model = get_models(PATH_TO_MODELS)   
    ...
    df_pred = preproceccing_df(sales_df_train, 
                            ...
                            is_train = False,
                            **df_best_dict)  
    predict = prediction_submitions(df_pred, model, sales_submission_df)     
    predict.to_csv(PATH_TO_SAVE_SUMISSION_DF, index=False)
